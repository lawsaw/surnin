/*
 * jQuery liScrollToTop v 1.0
 *
 * Copyright 2012, Linnik Yura | MASS CODE
 * Free to use
 * 
 * September 2012
 */
/* Attention! */
/* All, loaded with the plugin, images must have the attribute "height" and the attribute "data-src" with link to the original image! */
/* Внимание! */
/* Все, подгружаемые с помощью плагина, изображения  обязательно должны иметь атрибут "height" и атрибут "data-src" с оригинальной ссылкой на изображение! */

(function($){
    $.fn.liImageLoad = function(){
        return this.each(function(){
            var imgEl = $(this),
                imgElHeight = imgEl.attr('height'),
                imgElSrc = imgEl.data('src'),
                imgElPosTop = imgEl.offset().top,
                imgElPosBot = (imgElPosTop + imgEl.outerHeight()),
                wH = $(window).height(),
                wT = $(window).scrollTop(),
                wB = (wT + wH),
                f1 = false,
                data = 'data:image/gif;base64,'

            imgEl.attr('src',data)

            // if(imgElHeight == undefined){
            //     alert('Element ['+imgElSrc+'] contains no attribute "height"')
            // }
            // if(imgElHeight == ''){
            //     alert('The element ['+imgElSrc+'] has an empty attribute "height"')
            // }
            function imgElPos(){
                if(f1 == false){
                    if(imgElPosBot > wT && imgElPosTop < wB){
                        f1 = true;
                        imgEl.attr('src',imgElSrc).on('load',function(){
                            imgEl.css({opacity:'0'}).animate({opacity:'1'});
                            imgEl.addClass('imgLoaded');
                        });
                    }
                }
            }
            imgElPos();
            $(window).on('scroll',function(){
                wH = $(window).height();
                wT = $(window).scrollTop();
                wB = (wT + wH);
                imgElPos()
            })
        });
    };
})(jQuery);


var imgLoad = {

    'init': function() {
        var image = $(document).find('img[data-src]:not(.imgLoaded)');
        image.liImageLoad();
    },

};